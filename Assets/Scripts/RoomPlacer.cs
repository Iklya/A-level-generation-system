...
public class RoomPlacer
{
    ...
    private void GenerateGameMainRooms()
    {
        for (int r = 0; r < N; r++)
        {
            ...
            if (mainPath[r].RoomType == RoomType.Corridor)
            {
                ...
                if (curDoorAmount > 2)
                {
                    Debug.Log($"{curDoorAmount} - доступно дверей дл€ размещени€ в текущем коридоре");
        ...
        Debug.Log($"—оздалось: {GameObject.Find("MainPathRooms").transform.childCount} комнат основного пути");
    }
    ...
    private void GenerateGameExtraRooms(int corridorIdx)
    {
        ...
        foreach (var extra in extraPaths[corridorIdx])
        {
            ...
            // задаЄм им€ контейнеру (можно в отдельную функцию потом тоже, но есть ли смысл)
